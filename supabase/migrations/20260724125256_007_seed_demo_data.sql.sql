/*
# 007 — Seed demo data for FleetControl 360

## What this seeds
1. Three subscription plans (Starter / Professionnel / Entreprise).
2. One demo organization: "Abidjan Fleet Corp" (AFC) with 2 branches and 3 agencies.
3. Demo auth users with known passwords (all: Demo1234!).
4. Roles with permission sets + assignments + agency access.
5. Vehicle categories, 18 vehicles, vehicle documents (some expiring/expired).
6. 12 drivers, 12 clients, 8 suppliers.
7. ~10 rentals, 6 missions, 12 fuel entries, maintenance, expenses, invoices, payments.
8. Accidents, incidents, fines, GPS devices + demo positions (flagged is_demo).
9. Notifications and audit log entries.

## Demo credentials
admin@fleetcontrol360.com  / Demo1234!  → Super administrateur
director@afc.ci            / Demo1234!  → Admin entreprise
parc@afc.ci                / Demo1234!  → Responsable parc
finance@afc.ci             / Demo1234!  → Comptable
agent@afc.ci               / Demo1234!  → Agent de location
*/

-- ============================================================
-- SUBSCRIPTION PLANS
-- ============================================================
insert into public.subscription_plans (code, name, description, max_vehicles, max_users, max_agencies, price_monthly, price_yearly, features)
values
  ('starter', 'Starter', 'Fonctions essentielles pour petits parcs', 10, 5, 1, 25000, 250000,
    '{"modules":["vehicles","drivers","clients","rentals","basic_dashboard"],"support":"email"}'::jsonb),
  ('professional', 'Professionnel', 'Gestion complète avec finances, maintenance et GPS', 100, 25, 5, 75000, 750000,
    '{"modules":["vehicles","drivers","clients","rentals","missions","maintenance","fuel","finance","gps","driver_portal"],"support":"priority_email"}'::jsonb),
  ('enterprise', 'Entreprise', 'Solution illimitée multi-filiales avec API', 999999, 999999, 999999, 200000, 2000000,
    '{"modules":["all","multi_branch","api","integrations","custom"],"support":"dedicated"}'::jsonb)
on conflict (code) do nothing;

-- ============================================================
-- ORGANIZATION + BRANCHES + AGENCIES
-- ============================================================
insert into public.organizations (id, name, legal_name, tax_id, address, phone, email, currency_code, timezone, locale, primary_color, is_active)
values ('11111111-1111-1111-1111-111111111111', 'Abidjan Fleet Corp', 'Abidjan Fleet Corp SARL', 'CI-AFC-1234567',
  'Boulevard Latrille, Cocody, Abidjan', '+225 27 22 44 55 66', 'contact@afc.ci', 'XOF', 'Africa/Abidjan', 'fr', '#1e40af', true)
on conflict (id) do nothing;

insert into public.subscriptions (organization_id, plan_id, status, started_at, is_trial)
select '11111111-1111-1111-1111-111111111111', id, 'trialing', now(), true
from public.subscription_plans where code = 'professional'
and not exists (select 1 from public.subscriptions where organization_id = '11111111-1111-1111-1111-111111111111');

insert into public.branches (id, organization_id, name, code, address, phone, is_active) values
  ('22222222-1111-1111-1111-111111111111', '11111111-1111-1111-1111-111111111111', 'Filiale Abidjan', 'ABJ', 'Cocody, Abidjan', '+225 27 22 44 55 66', true),
  ('22222222-2222-2222-2222-222222222222', '11111111-1111-1111-1111-111111111111', 'Filiale Intérieur', 'INT', 'Bouaké', '+225 27 31 66 77 88', true)
on conflict (id) do nothing;

insert into public.agencies (id, organization_id, branch_id, name, code, city, country, address, phone, manager_name, is_active) values
  ('33333333-1111-1111-1111-111111111111', '11111111-1111-1111-1111-111111111111', '22222222-1111-1111-1111-111111111111', 'Agence Cocody', 'ABJ-COC', 'Abidjan', 'Côte d''Ivoire', 'Rue des Jardins, Cocody', '+225 07 00 11 22 33', 'Konan Yao', true),
  ('33333333-2222-2222-2222-222222222222', '11111111-1111-1111-1111-111111111111', '22222222-1111-1111-1111-111111111111', 'Agence Yopougon', 'ABJ-YOP', 'Abidjan', 'Côte d''Ivoire', 'Boulevard Giscard, Yopougon', '+225 07 00 11 22 34', 'Aya Koné', true),
  ('33333333-3333-3333-3333-333333333333', '11111111-1111-1111-1111-111111111111', '22222222-2222-2222-2222-222222222222', 'Agence Bouaké', 'BKE', 'Bouaké', 'Côte d''Ivoire', 'Centre-ville, Bouaké', '+225 07 00 11 22 35', 'Moussa Touré', true)
on conflict (id) do nothing;
...
declare
  ...
  u_platform uuid := 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa';
  u_director  uuid := 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb';
  u_parc      uuid := 'cccccccc-cccc-cccc-cccc-cccccccccccc';
  u_finance   uuid := 'dddddddd-dddd-dddd-dddd-dddddddddddd';
  u_agent     uuid := 'eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee';
...
begin
  ...
  select u, 'authenticated', 'authenticated', e, pw, now(), '{}'::jsonb, jsonb_build_object('full_name', fn), now(), now(), '', ''
  from (values
    (u_platform, 'admin@fleetcontrol360.com', 'Super Admin'),
    (u_director,  'director@afc.ci', 'Awa Bamba'),
    (u_parc,      'parc@afc.ci', 'Yao Konan'),
    (u_finance,   'finance@afc.ci', 'Fatou Diarra'),
    (u_agent,     'agent@afc.ci', 'Koffi N''Guessan')
  ) as x(u, e, fn)
  on conflict (id) do nothing;

  insert into public.user_profiles (id, organization_id, email, full_name, is_platform_admin, is_active)
  values
    (u_platform, null, 'admin@fleetcontrol360.com', 'Super Admin', true, true),
    (u_director, org, 'director@afc.ci', 'Awa Bamba', false, true),
    (u_parc, org, 'parc@afc.ci', 'Yao Konan', false, true),
    (u_finance, org, 'finance@afc.ci', 'Fatou Diarra', false, true),
    (u_agent, org, 'agent@afc.ci', 'Koffi N''Guessan', false, true)
  ...
  select gen_random_uuid(), u, u::text, 'email', jsonb_build_object('email', e, 'sub', u::text), now(), now(), now()
  from (values
    (u_platform, 'admin@fleetcontrol360.com'),
    (u_director, 'director@afc.ci'),
    (u_parc, 'parc@afc.ci'),
    (u_finance, 'finance@afc.ci'),
    (u_agent, 'agent@afc.ci')
  ) as x(u, e)
...
select x.u, r.id, '11111111-1111-1111-1111-111111111111'
from public.roles r
cross join (values
  ('bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb'::uuid, 'org_admin'),
  ('cccccccc-cccc-cccc-cccc-cccccccccccc'::uuid, 'parc_manager'),
  ('dddddddd-dddd-dddd-dddd-dddddddddddd'::uuid, 'accountant'),
  ('eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee'::uuid, 'rental_agent')
) as x(u, code)
where r.organization_id = '11111111-1111-1111-1111-111111111111' and r.code = x.code
on conflict (user_id, role_id) do nothing;

insert into public.user_agency_access (user_id, agency_id, can_view, can_manage)
select x.u, a.id, true, true
...
-- ============================================================
insert into public.application_settings (organization_id, settings)
values ('11111111-1111-1111-1111-111111111111',
  '{"currency":"XOF","timezone":"Africa/Abidjan","locale":"fr","invoicePrefix":"FAC-","quotePrefix":"DEV-","rentalPrefix":"LOC-","missionPrefix":"MIS-","alertDays":[90,60,30,15,7,0],"defaultDeposit":150000,"latePenaltyPerDay":10000,"fuelAnomalyThreshold":1.5}'::jsonb)
...
select '11111111-1111-1111-1111-111111111111', x.name, x.code, x.description
from (values
  ('Berline','berline','Véhicule de tourisme standard'),
  ('SUV','suv','Véhicule utilitaire sport'),
  ('4x4','4x4','Véhicule tout terrain'),
  ('Minibus','minibus','Transport de groupe'),
  ('Camion','camion','Transport de marchandises'),
  ('Moto','moto','Deux-roues'),
  ('Ambulance','ambulance','Transport sanitaire'),
  ('Véhicule blindé','blinde','Véhicule sécurisé'),
...
select '11111111-1111-1111-1111-111111111111', a.id, c.id, x.*
from (values
  ('ABJ-001','AB-1234-AB','VF1BX123456','Toyota','Corolla','berline','berline','gris',2021,'essence',50.0,6.5,'manuelle',5,42000,'owned',null,12000000,8000000,200000,'available','available','Bon état général'),
  ('ABJ-002','AB-5678-CD','WVWZZZ12345','Volkswagen','Passat','berline','berline','noir',2020,'diesel',60.0,5.8,'automatique',5,68000,'owned',null,15000000,9000000,250000,'rented','unavailable','En location client BTP'),
  ('ABJ-003','AB-9012-EF','JM1NC123456','Mazda','CX-5','suv','suv','blanc',2022,'essence',58.0,7.2,'automatique',5,21000,'owned',null,18000000,13000000,300000,'available','available','SUV premium'),
  ('ABJ-004','AB-3456-GH','JTMHV123456','Toyota','Hilux','4x4','4x4','rouge',2019,'diesel',80.0,9.0,'manuelle',5,95000,'owned',null,22000000,11000000,350000,'on_mission','unavailable','Mission escorte Abidjan'),
  ('ABJ-005','AB-7890-IJ','WBAPH123456','BMW','X5','suv','suv','bleu',2021,'diesel',75.0,8.5,'automatique',5,38000,'owned',null,35000000,22000000,500000,'maintenance','unavailable','Vidange + freins'),
  ('YOP-001','AB-2345-JK','MALBF12345','Mitsubishi','Pajero','4x4','4x4','vert',2018,'diesel',88.0,10.0,'manuelle',5,140000,'owned',null,25000000,9000000,400000,'available','available','4x4 robuste'),
  ('YOP-002','AB-6789-LM','KMHMH12345','Hyundai','H1','minibus','minibus','blanc',2020,'diesel',75.0,8.0,'manuelle',9,72000,'owned',null,20000000,12000000,300000,'rented','unavailable','Location navette entreprise'),
  ('YOP-003','AB-0123-NO','WBAVB12345','Mercedes','Classe C','berline','berline','noir',2021,'diesel',66.0,5.5,'automatique',5,31000,'owned',null,28000000,18000000,400000,'available','available','Véhicule de direction'),
  ...
  ('BKE-001','AB-8901-RS','VF7BA12345','Peugeot','Partner','camionnette','camionnette','blanc',2019,'diesel',60.0,6.0,'manuelle',3,110000,'owned',null,9000000,4500000,150000,'available','available','Utilitaire logistique'),
  ('BKE-002','AB-2345-TU','JTFHT12345','Isuzu','NPR','camion','camion','jaune',2016,'diesel',140.0,15.0,'manuelle',3,210000,'owned',null,30000000,8000000,400000,'repair','unavailable','Réparation moteur garage'),
  ('BKE-003','AB-6789-VW','MBHKR12345','Ford','Ranger','4x4','4x4','argent',2020,'diesel',80.0,9.5,'manuelle',5,88000,'owned',null,26000000,14000000,350000,'available','available','4x4 chantier BTP'),
  ('ABJ-006','AB-9012-XY','TMNKJ12345','Toyota','Hiace','minibus','minibus','blanc',2021,'diesel',70.0,8.0,'manuelle',15,45000,'owned',null,19000000,13000000,300000,'reserved','unavailable','Réservé client ONG'),
  ('ABJ-007','AB-3456-YZ','WDDZF12345','Mercedes','GLE','suv','suv','noir',2022,'diesel',80.0,8.8,'automatique',5,18000,'owned',null,45000000,32000000,600000,'available','available','SUV VIP'),
  ('ABJ-008','AB-7890-AC','1FTFW12345','Ford','Everest','suv','suv','gris',2021,'diesel',80.0,9.0,'automatique',7,36000,'owned',null,30000000,20000000,450000,'accident','unavailable','Accident voie rapide'),
  ('YOP-005','AB-0123-BD','KMHHM12345','Hyundai','Tucson','suv','suv','rouge',2020,'essence',62.0,7.5,'automatique',5,58000,'owned',null,16000000,10000000,250000,'available','available',null),
  ('BKE-004','AB-4567-EF','9BWZZZ12345','Volkswagen','Amarok','4x4','4x4','blanc',2020,'diesel',80.0,9.2,'manuelle',5,79000,'owned',null,28000000,16000000,400000,'available','available',null),
  ('ABJ-009','AB-8901-GH','TMABZ12345','Toyota','Land Cruiser','4x4','4x4','blanc',2022,'diesel',93.0,11.0,'automatique',8,12000,'owned',null,55000000,42000000,700000,'on_mission','unavailable','Escorte VIP sécurisée')
...
left join public.agencies a on a.code = split_part(x.internal_number, '-', 1)
left join public.vehicle_categories c on c.code = x.category and c.organization_id = '11111111-1111-1111-1111-111111111111'
where a.organization_id = '11111111-1111-1111-1111-111111111111'
on conflict do nothing;

-- ============================================================
-- VEHICLE DOCUMENTS (some expiring soon / expired)
-- ============================================================
...
select '11111111-1111-1111-1111-111111111111', a.id, x.*
from (values
  ('CHF-001','Koffi','Yao','M','1985-05-12'::date,'+225 07 11 22 33','Cocody Abidjan','+225 05 99 88 77','2019-03-01'::date,'CDI','available','PERM-001','B,C,EC','2010-04-01'::date,'2025-04-01'::date,12,'Français, Baoulé',250000,50000,4.8,'Premiers secours','Chauffeur expérimenté'),
  ('CHF-002','Awa','Bamba','F','1990-08-23'::date,'+225 07 11 22 34','Yopougon Abidjan','+225 05 99 88 76','2020-06-15'::date,'CDI','on_mission','PERM-002','B','2012-05-01'::date,'2025-05-01'::date,10,'Français, Anglais',220000,40000,4.9,'VIP escort','Spécialiste escorte'),
  ('CHF-003','Moussa','Touré','M','1988-01-30'::date,'+225 07 11 22 35','Bouaké','+225 05 99 88 75','2018-01-10'::date,'CDI','on_mission','PERM-003','B,C,EC,ED','2008-02-01'::date,'2024-10-15'::date,15,'Français, Dioula',230000,45000,4.5,null::text,'Permis à renouveler'),
  ('CHF-004','Ibrahim','Cissé','M','1992-11-05'::date,'+225 07 11 22 36','Adjamé Abidjan','+225 05 99 88 74','2021-09-01'::date,'CDD','available','PERM-004','B,C','2015-03-01'::date,'2026-03-01'::date,8,'Français, Bambara',200000,30000,4.2,null::text,null),
  ('CHF-005','Aminata','Koné','F','1995-03-18'::date,'+225 07 11 22 37','Treichville Abidjan','+225 05 99 88 73','2022-02-01'::date,'CDD','resting','PERM-005','B','2018-06-01'::date,'2026-06-01'::date,6,'Français',180000,25000,4.6,null::text,'En repos'),
  ('CHF-006','Sékou','Camara','M','1983-07-22'::date,'+225 07 11 22 38','Koumassi Abidjan','+225 05 99 88 72','2016-05-01'::date,'CDI','on_mission','PERM-006','B,C,EC,EB','2005-01-01'::date,'2027-01-01'::date,18,'Français, Malinké',280000,60000,4.7,'Transport marchandises','Chauffeur poids lourd'),
  ('CHF-007','Fatim','Zerbo','F','1991-12-10'::date,'+225 07 11 22 39','Cocody Abidjan','+225 05 99 88 71','2020-11-01'::date,'CDI','available','PERM-007','B','2013-09-01'::date,'2025-09-01'::date,11,'Français, Mooré',210000,35000,4.8,null::text,null),
  ('CHF-008','Adama','Traoré','M','1987-04-25'::date,'+225 07 11 22 40','Yopougon Abidjan','+225 05 99 88 70','2017-08-01'::date,'CDI','suspended','PERM-008','B,C','2009-07-01'::date,'2026-07-01'::date,14,'Français, Bambara',240000,30000,3.9,null::text,'Suspendu pour retard'),
  ('CHF-009','Mariam','Sangaré','F','1998-06-15'::date,'+225 07 11 22 41','Bouaké','+225 05 99 88 69','2023-01-15'::date,'Stage','available','PERM-009','B','2021-04-01'::date,'2026-04-01'::date,3,'Français',150000,15000,4.3,null::text,'Jeune chauffeur'),
  ('CHF-010','Bakary','Doumbia','M','1982-09-30'::date,'+225 07 11 22 42','Bouaké','+225 05 99 88 68','2015-02-01'::date,'CDI','on_mission','PERM-010','B,C,EC,ED,EB','2003-03-01'::date,'2028-03-01'::date,21,'Français, Bambara, Dioula',300000,70000,4.6,'Transport long distance','Chauffeur senior'),
  ('CHF-011','Eric','Kouadio','M','1994-02-14'::date,'+225 07 11 22 43','Cocody Abidjan','+225 05 99 88 67','2021-04-01'::date,'CDI','absent','PERM-011','B','2016-08-01'::date,'2025-08-01'::date,9,'Français',200000,25000,4.0,null::text,'Absent maladie'),
  ('CHF-012','Christelle','Adjoua','F','1996-10-08'::date,'+225 07 11 22 44','Yopougon Abidjan','+225 05 99 88 66','2022-08-01'::date,'CDI','available','PERM-012','B,C','2019-12-01'::date,'2027-12-01'::date,7,'Français, Bété',190000,30000,4.7,null::text,null)
) as x(matricule, first_name, last_name, gender, birth_date, phone, address, emergency_contact, hire_date, contract_type, status, license_number, license_category, license_issue_date, license_expiry_date, experience_years, languages, salary, bonus, rating, certifications, notes)
left join public.agencies a on a.city = case when x.matricule in ('CHF-009','CHF-010') then 'Bouaké' else 'Abidjan' end
where (a is null or a.organization_id = '11111111-1111-1111-1111-111111111111')
on conflict do nothing;
...
select '11111111-1111-1111-1111-111111111111', x.type, x.name, x.representative, x.contact_person, x.phone, x.email, x.address, x.tax_id, x.trade_register, x.credit_limit, x.payment_delay_days, x.risk_level, x.account_status, x.notes
from (values
  ('company','BTP Construction Plus SARL','Marc Kouadio','Marc Kouadio','+225 27 22 33 44 55','contact@btpplus.ci','Zone 4, Abidjan','CI-BTP-4455','CI-ABJ-2018-123',5000000,30,'low','active','Client fidèle, 3-4 locations/mois'),
  ('company','Groupe Hôtelier Azur','Sophia Bamba','Sophia Bamba','+225 27 22 33 44 56','s.bamba@azur-hotels.com','Bingerville','CI-AZU-5566',null::text,3000000,15,'low','active','Navette aéroport régulière'),
  ('individual','Jean-Marc Aka',null::text,'Jean-Marc Aka','+225 07 77 88 99 00','jm.aka@gmail.com','Cocody Riviera 3',null::text,null::text,500000,0,'low','active','Client particulier premium'),
  ('administration','Ministère des Transports','Directeur de Cabinet','Adjoua Koné','+225 27 22 33 44 57','cabinet@transports.gouv.ci','Plateau, Abidjan',null::text,null::text,10000000,45,'low','active','Missions administratives'),
  ('ngo','Croix Rouge Côte d''Ivoire','Coordonnateur Logistique','Ibrahim Cissé','+225 27 22 33 44 58','logistique@crci.org','Cocody Angré',null::text,null::text,4000000,30,'low','active','Transport humanitaire'),
  ('company','Société SecurPro SARL','CEO','Paul Gnagne','+225 07 77 88 99 01','p.gnagne@securpro.ci','Marcory Zone 3','CI-SEC-7788','CI-ABJ-2019-456',8000000,15,'medium','active','Escorte VIP'),
  ('individual','Aminata Traoré',null::text,'Aminata Traoré','+225 07 77 88 99 02','a.traore@yahoo.fr','Yopougon Selmer',null::text,null::text,300000,0,'low','active','Location week-end'),
  ('company','TransPlus Logistique','DG','Bakary Fofana','+225 27 22 33 44 59','b.fofana@transplus.ci','Vridi',null::text,'CI-ABJ-2017-789',6000000,30,'medium','active','Convoyage marchandises'),
  ('company','Eventis Organisation','Manager','Christelle Aya','+225 07 77 88 99 03','c.aya@eventis.ci','Cocody',null::text,'CI-ABJ-2020-321',2000000,15,'high','active','Événementiel, paiements parfois en retard'),
  ('individual','Dr. Paul Yapi',null::text,'Dr. Paul Yapi','+225 07 77 88 99 04','p.yapi@med.ci','Cocody Riviera 2',null::text,null::text,1000000,0,'low','active','Médecin, location ponctuelle'),
  ('partner','Agence Voyage Discovery','Responsable','Roger Tanoh','+225 27 22 33 44 60','r.tanoh@discovery.ci','Plateau',null::text,null::text,5000000,15,'low','active','Partenaire touristique'),
  ('company','BuildMat Côte d''Ivoire','Directeur Logistique','Adama Soro','+225 27 22 33 44 61','a.soro@buildmat.ci','Yopougon',null::text,'CI-ABJ-2016-654',4000000,30,'medium','suspended','Retard de paiement')
) as x(type, name, representative, contact_person, phone, email, address, tax_id, trade_register, credit_limit, payment_delay_days, risk_level, account_status, notes)
on conflict do nothing;
...
select '11111111-1111-1111-1111-111111111111', x.type, x.name, x.contact_person, x.phone, x.email, x.address, x.services, x.rating, x.notes
from (values
  ('garage','Garage Central Toyota','Koffi Mécanicien','+225 27 22 88 11 22','garage.toyama@ci.com','Yopougon','Mécanique, vidange, freinage',4.5,'Garage principal'),
  ('garage','Garage Peugeot Cocody','Aboubakar','+225 27 22 88 11 23','garage.psa@ci.com','Cocody','Mécanique, électronique',4.2,null::text),
  ('insurer','NSIA Assurance','Agent commercial','+225 27 22 99 33 44','contact@nsia.ci','Plateau','Assurance auto tous risques',4.6,'Assureur principal'),
  ('insurer','AXA Côte d''Ivoire','Service sinistres','+225 27 22 99 33 45','sinistres@axa.ci','Plateau','Assurance auto et flotte',4.3,null::text),
  ('fuel_station','Station Total Cocody','Gérant','+225 27 22 77 55 66',null::text,'Cocody','Carburant essence/diesel',4.0,'Carte carburant AFC'),
  ('fuel_station','Station Pétro Ivoire Yopougon','Gérant','+225 27 22 77 55 67',null::text,'Yopougon','Carburant essence/diesel',3.9,null::text),
  ('parts_vendor','Pièces Auto Express','Vendeur','+225 07 55 66 77 88','vente@piecesexpress.ci','Adjamé','Pièces détachées toutes marques',4.1,null::text),
  ('gps_provider','TrackGPS Africa','Support technique','+225 27 22 66 44 33','support@trackgps.africa','Marcory','Traceurs GPS, géolocalisation',4.4,'Fournisseur GPS principal')
) as x(type, name, contact_person, phone, email, address, services, rating, notes)
on conflict do nothing;
...
-- ============================================================
-- GPS DEVICES + DEMO POSITIONS (flagged is_demo = true)
-- ============================================================
